...
export function NewsListFormEdit(props: ComponentProps) {
  ...
  const header = (
    <div>
      <img className="border-round" alt="Card" src={formik.values.thumbnail} />
    </div>
  );

  const subtitle = (
    <div className="flex flex-column p-2 gap-2">
      <div className="m-2">Published: {formik.values.publishedAt}</div>
      <h4 className="m-2">{formik.values.description}</h4>
    </div>
  );
  ...
  return (
    <div className="flex flex-column justify-content-center flex-wrap row-gap-6 p-8 pb-20rem">
      <div>
        {" "}
        <h1>Update News</h1>
      </div>
